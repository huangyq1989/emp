    ...
    <body>
        <div style="background: #fafafa; color:#080808;height:100%;width:100%">
            <div style="border:1px solid #808080;margin-bottom:-1px;text-align:center;background-color: #428bca;color: #0E2D5F;">
                <label style="font-size: 23px;color: white">${title}-试卷发布</label>
            </div>

            <div style="border:1px solid #808080">
                <div style="margin-top: 14%;text-align:center;">
                    <img class="q_code" src="${pageContext.request.contextPath}/examination/getTpQRCode.do?examinationId=${examinationId}&cultivateId=${cultivateId}" />
                </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        <c:choose>
            ...
            <c:otherwise>
                swal({title: '错误提示',text: "该考试没有试题,不可进行发布,请先添加试题!",type: "error",confirmButtonText: "确  定"}, function(){window.close();});
            </c:otherwise>
        </c:choose>
    });
    ...
    //下载二维码
    function downloadQR() {
        window.location.href = '${pageContext.request.contextPath}/examination/getTpQRCode.do?examinationId=${examinationId}&cultivateId=${cultivateId}&type=download&name=${title}试卷二维码';
    };
    ...
    function stopPublish(){
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/examination/updatePublishStatus.do?examinationId=${examinationId}&type=stop",
            traditional:true,
            async:false,
            dataType:"json",
