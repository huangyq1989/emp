    ...
    <body>
        <div style="background: #fafafa; color:#080808;height:100%;width:100%">
            <div style="border:1px solid #808080;margin-bottom:-1px;text-align:center;background-color: #428bca;color: #0E2D5F;">
                <label style="font-size: 23px;color: white">${title}-问卷复制</label>
            </div>
            <div style="border:1px solid #808080">
                <div style="padding:5px;">
                    <div style="margin-top: 4%;width:763px; height:709px">
                        <div style="margin-right: 21%;">
                            <label class="am-fl am-form-label" style="font-size:15px;margin-left: 2%;color: #00458a"><strong>说明 : 查询结果可能有部分是已有问卷的(红色标记),保存的话会覆盖掉以前的问卷信息.</strong></label>
                        </div>
                        <div style="margin-top: 12%;margin-left: 10%;">
                            <label class="am-fl am-form-label" style="font-size:19px;">评估名称:  </label>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        <c:choose>
            ...
            <c:otherwise>
                swal({title: '错误提示',text: "该培训没有问卷,不可进行复制,请先添加问卷!",type: "error",confirmButtonText: "确  定"}, function(){window.close();});
            </c:otherwise>
        </c:choose>
    ...
    //根据培训名称模糊查询
    function queryByName(estimateName) {
        document.getElementById('content').innerHTML = "";
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/estimate/queryByName.do",
            data: {"estimateName":estimateName.value},
            traditional:true,
            async:false,
            dataType:"json",
            success: function(data){
                ...
                for(var i=0;i<data.rows.length;i++){
                    var t_estimateId = ${estimateId};
                    if(data.rows[i].id != t_estimateId){ //去除自己,不能复制给自己
                        ...
                        temp_input.setAttribute("value",data.rows[i].estimateName.replace("(已有问卷)",""));

                        var temp_label = document.createElement("label");
                        temp_label.setAttribute("class","am-fl am-form-label");
                        if(data.rows[i].estimateName.indexOf("(已有问卷)") != -1){
                        ...
                        temp_label.innerText = "[序号:"+data.rows[i].id+"] "+data.rows[i].estimateName;
    ...
    function save() {
        ...
        if(obj.length == 0){
            if(inputName == ""){
                swal({title: "错误提示",text: "请输入评估名称",type: "error",confirmButtonText: "确  定"});
                return false;
            }
            //没查询到属于新增
            type = "add" ;
            ...
            if(arrs.length == 0){
                ...
                for(var k in obj){
                    if(inputName == obj[k].value){
                        swal({title: "错误提示",text: "请勾选同名的评估",type: "error",confirmButtonText: "确  定"});
                        return false;
                    }
                }
                type = "add" ;
        ...
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/estimate/saveCopyQuestionnaire.do?estimateId=${estimateId}&type="+type+"&inputName="+inputName,
            data: {"arrs":arrs},
            traditional:true,
            async:false,
            dataType:"json",
