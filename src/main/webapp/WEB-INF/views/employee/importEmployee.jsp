    ...
    <body>
        <div style="background: #fafafa; color:#080808">
            <div style="border:1px solid #808080;margin-bottom:-1px;text-align:center;background-color: #428bca;color: #0E2D5F;">
                <label style="font-size: 23px;color: white">账号导入</label>
            </div>
                ...
                <div style="text-align: center; padding:15px;height: 400px;overflow:auto;">
                    <form id="formMessage" name="formMessage" style="padding: 0px 80px 20px 20px;">
                        <table width="100%" width="700" height="50" border="1" bordercolor="#000000" style="border-collapse:collapse" id="resultTable">
                            <tr bgcolor="#b0e0e6"><td>序号</td><td>姓名</td><td>登录账号</td><td>手机号码</td><td>校验结果</td></tr>
                        </table>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    //下载模板
    function download() {
        window.location.href = '${pageContext.request.contextPath}/importModel/账号导入模板.xlsx';
    };
    ...
    function importAndCheck(){
        ...
        if(fileName == "" || fileName == null){
            swal({title: "错误提示",text: "请先选择需要导入的账号模板",type: "error",confirmButtonText: "确  定"});
            return false;
        }
        ...
        if (filesList.length == 0) {//如果取消上传，则改文件的长度为0
            ...
            if (filesList[0].size > 5000000) {
                ...
                $.ajax({
                    url : "${pageContext.request.contextPath}/fileUpload/UploadServlet?type=employee",
                    type: 'POST',
                    data: formData,
                    async: true,
                    cache: false,
                    ...
                    success : function(data) {
                        ...
                        if (result.flag == 0) {
                            ...
                            for(i=0;i<result.list.length;i++){
                                ...
                                three.innerHTML = result.list[i].userName;
    ...
    function saveImportData(){
        ...
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/employee/saveImportData.do",
            data: {"sendData":param},
            traditional:true,
            async:true,
            dataType:"json",
